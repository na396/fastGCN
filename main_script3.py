...
for ds in dataset_name:
    ...
    dic_dir = root_dir + "Results/dic.pickle"
    try:
        final = pickle.load(open(dic_dir, "rb"))
    except (OSError, IOError, EOFError) as e:
        final = {}

    final[str(graph.graph_name) + str(script_number)] = res
    pickle.dump(final, open(dic_dir, "wb"))

    graph_dir = root_dir + "Results/graphs.dic"
    try:
        graphs = pickle.load(open(graph_dir, "rb"))
    except (OSError, IOError, EOFError) as e:
        graphs = {}

    graphs[str(graph.graph_name) + str(script_number)] = graph
    pickle.dump(graphs, open(graph_dir, "wb"))
